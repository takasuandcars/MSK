class RecordPdf < Prawn::Document
  ...
  def initialize(record)
    ...
    @timenew = Time.new
    @timetokyo = @timenew.in_time_zone('Tokyo')
    @time = @timetokyo.since(2.days)
    ...
    text_box "#{@timetokyo.strftime('%Y年%-m月%-d日')}", size: 13, at: [470, 680], valign: :center, height: 15
    ...
    text_box "#{@record.car_type}   #{@record.number_of_car}台", size: 13, at: [75, 600], height: 30, valign: :center
    ...
    text_box '千葉県山武郡芝山町岩山2315-23   丸全昭和運輸株式会社 E棟   成田物流センター  C/O DHLグローバルフォワーディングジャパン株式会社', size: 13, at: [75, 540],
                                                                                            width: 320, height: 60, valign: :center, leading: 3
    ...
    text_box "#{@record.pickup_date.strftime('%Y年%-m月%-d日')} or #{@record.pickup_date.since(1.days).strftime('%-m月%-d日')}朝",
             size: 13, at: [75, 375], height: 30, valign: :center
    ...
    text_box "#{@record.weight} ", size: 13, at: [360, 200], height: 30, valign: :center
